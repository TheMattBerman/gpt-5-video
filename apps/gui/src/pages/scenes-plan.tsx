...
export default function ScenesPlanPage() {
  ...
  const validateLine = useMemo(() => ajv.compile(sceneSpecsLineSchema as any), []);
  ...
  function validate(input: string) {
    try {
      ...
      for (const s of asArray) {
        if (!validateLine(s)) {
          setErrors((validateLine.errors || []).map((e) => `${e.instancePath || "/"} ${e.message || "invalid"}`));
    ...
    <main className="min-h-dvh bg-gray-50">
      <div className="mx-auto max-w-5xl p-6 space-y-6">
        ...
        <section className="rounded border bg-white p-4">
          <div className="mb-3 text-sm text-gray-700">Author one scene spec or an array, validate, then submit.</div>
            ...
            <div className="flex flex-col">
              ...
              <div className="mt-3 flex items-center gap-3">
                <button onClick={submit} disabled={errors.length > 0}
                        className="rounded bg-black px-3 py-1.5 text-white text-sm disabled:opacity-50">Submit</button>
              </div>
              {result && (
                <pre className="mt-3 max-h-60 overflow-auto rounded border bg-gray-50 p-2 text-xs">{JSON.stringify(result, null, 2)}</pre>
              )}
            </div>
          </div>
        </section>
...
const exampleScene: SceneSpec = {
  ...
  audio: {
    dialogue: [{ character: "Mascot", line: "Your CAC is fine. Your loop is broken." }],
    vo_prompt: ""
  }
};
