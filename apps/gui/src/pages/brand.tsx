...
export default function BrandIngestPage() {
  ...
  const [jsonText, setJsonText] = useState<string>(() => JSON.stringify(exampleBrandProfile, null, 2));
  ...
  function validate(text: string) {
    try {
      ...
      setErrors((validator.errors || []).map((e: any) => `${e.instancePath || "/"} ${e.message || "invalid"}`));
    ...
    <main className="min-h-dvh bg-gray-50">
      <div className="mx-auto max-w-5xl p-6 space-y-6">
        ...
        <section className="rounded border bg-white p-4">
          <div className="mb-3 text-sm text-gray-700">Paste or edit a `brand_profile.json` then validate and submit.</div>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex flex-col">
              ...
              <div className="mt-3 flex items-center gap-3">
                ...
                {resultId && <div className="text-sm">Created id: <span className="font-mono">{resultId}</span></div>}
              </div>
            </div>
          </div>
        </section>
